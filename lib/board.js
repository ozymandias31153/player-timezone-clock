...
const ZONE_LIBRARY = [
  ...
  { id: "uk", label: "UK", tz: "Europe/London", defaultEnabled: true, order: 10 },
  { id: "shanghai", label: "Shanghai", tz: "Asia/Shanghai", defaultEnabled: true, order: 11 },
  { id: "bulgaria", label: "Bulgaria", tz: "Europe/Sofia", defaultEnabled: false, order: 12 },
  { id: "samara", label: "Samara", tz: "Europe/Samara", defaultEnabled: true, order: 13 }
];
...
function buildControlRows(board) {
  ...
  const enabledCount = getEnabledCards(board).length;
  ...
  return [
    {
      ...
      components: [
        ...
        button("board:reorder", "Reorder", 2, enabledCount < 2)
      ]
    },
    {
      type: 1,
...
function buildReorderControls(cardId, cards) {
  const enabledCards = Array.isArray(cards) ? cards : [];
  const currentIndex = enabledCards.findIndex((card) => card.id === cardId);
  const atTop = currentIndex <= 0;
  const atBottom = currentIndex < 0 || currentIndex >= enabledCards.length - 1;

  return {
    type: 1,
    components: [
      button(`board:reorder:move:${cardId}:top`, "Move to Top", 2, atTop),
      button(`board:reorder:move:${cardId}:up`, "Move Up", 1, atTop),
      button(`board:reorder:move:${cardId}:down`, "Move Down", 1, atBottom),
      button(`board:reorder:move:${cardId}:bottom`, "Move to Bottom", 2, atBottom),
      button("board:reorder:done", "Done", 3, false)
    ]
  };
}
